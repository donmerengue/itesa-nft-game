...
const Items = ({ nft, active, setLoaded }) => {
  const [nftData, setNftData] = useState({});
  const [nftImage, setNftImage] = useState("");
  const [comprados, setComprados] = useState(null);
  const [renderizar, setRenderizar] = useState(undefined);

  useEffect(() => {
    setNftData(JSON.parse(nft.metadata));
    getData("nftBought").then((res) => {
      setComprados(res);
    });
  }, [active]);
  ...
  useEffect(() => {
    if (comprados && nft) {
      let filtrados = comprados.find(
        (nftMios) => nftMios.tokenId === nft.token_id
      );
      setRenderizar(filtrados);
    }
  }, [comprados]);

  return renderizar?.name ? (
    ""
  ) : (
    <>
      {nftData?.attributes ? (
